...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/06/28 15:19:48 by elopukh           #+#    #+#             */
/*   Updated: 2018/06/28 15:19:50 by elopukh          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fract.h"

void	iter_s(double pr, double pi, t_window *win, int maxiter)
{
	double	new_re;
	double	new_im;
	double	old_re;
	...
	while (i < maxiter && (new_re * new_re + new_im * new_im) < 4)
	{
		old_re = new_re;
		old_im = new_im;
		new_re = fabs(pow(old_re, 3) - 3 * old_im * old_im + pr);
		new_im = fabs(3 * old_re * old_re * old_im - pow(old_im, 3) + pi);
		i++;
	}
	if (i == maxiter)
...
void	count_ship(t_window *win)
{
	...
	i = 0;
	while (++i <= 20)
		pthread_create(&t[i - 1], NULL, ship, &tab[i - 1]);
	while (i--)
	{
